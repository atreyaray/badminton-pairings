...
export function MatchesPage({ initialSession }: MatchesPageProps) {
  ...
  const handleMatchComplete = (matchId: string) => {
    ...
    // Generate new match if we have fewer active matches than courts
    const activeMatchesCount = matchStates.filter(ms => !ms.completed && ms.match.id !== matchId).length
    if (activeMatchesCount < session.numberOfCourts) {
      const newSession = generateSession(
        session.players,
        session.numberOfCourts - activeMatchesCount,
        updatedCompletedMatches
      )
      
      // Add new matches to the session without modifying existing ones
      setSession(prev => ({
        ...prev,
        matches: [...prev.matches, ...newSession.matches]
      }))
      
      // Only add new match states for the new matches
      setMatchStates(prev => [
        ...prev,
        ...newSession.matches.map(match => ({
          match,
          completed: false
        }))
      ])
